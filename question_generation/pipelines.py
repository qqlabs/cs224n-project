...
class QGPipeline:
    ...
    def __call__(self, inputs: str):
        ...
        output = [{'question': que, 'id': hashlib.md5((que + example['answer']).encode('utf-8')).hexdigest(), 'answers': [{"answer_start": example['answer_start'], "text": example['answer']}]} for example, que in zip(qg_examples, questions)]
        return output
    ...
    def _prepare_inputs_for_qg_from_answers_hl(self, sents, answers):
        ...
        for i, answer in enumerate(answers):
            ...
            for answer_text in answer:
                # sent = sents[i]
                sents_copy = " ".join(sents[:])

                answer_text = answer_text.replace('<pad>','').strip()
                
                # find all instances where we match the answer_text
                matches = [m.start() for m in re.finditer(re.escape(answer_text.lower()), sents_copy.lower())]

                # currently might have weird cases where answer is 'a book' but string is 'an ancient book'
                # so we don't have exact match
                # we'll just skip these examples for now
                if len(matches) == 0:
                    continue

                closest_sent_idx = sents_copy.index(sents[i])

                # pick the instance that is closest to the current generated answer idx
                ans_start_idx = matches[(np.abs(np.array(matches) - closest_sent_idx)).argmin()]

                source_text = f"generate question: {sents_copy[:ans_start_idx]} <hl> {answer_text} <hl> {sents_copy[ans_start_idx + len(answer_text): ]}"
                ...
                inputs.append({"answer_start": ans_start_idx, "answer": answer_text, "source_text": source_text})
